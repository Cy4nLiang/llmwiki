...
def main(argv=None):
    ...
    # log 模板若自带 bootstrap 占位条目(`## [YYYY-MM-DD] bootstrap | …`):
    # 确定性地替换为 --date 并删除操作指引注释;后续检测到已有 bootstrap 条目则不再追加兜底条目。
    for idx, (rel, out) in enumerate(rendered):
        if rel != "wiki/log.md":
            continue
        lines = []
        for ln in out.splitlines():
            if re.match(r"^\s*<!--.*bootstrap.*-->\s*$", ln):
                continue  # 模板内「替换日期后删除本注释」类指引,渲染时代为执行
            if ln.startswith("## [YYYY-MM-DD] bootstrap"):
                ln = ln.replace("[YYYY-MM-DD]", "[%s]" % date, 1)
            lines.append(ln)
        rendered[idx] = (rel, "\n".join(lines) + "\n")

    # wiki meta 页 frontmatter 的 created/updated 日期占位 → --date(仅 frontmatter 区,W-PAGE-4)
    for idx, (rel, out) in enumerate(rendered):
        if not rel.startswith("wiki/") or rel == "wiki/log.md":
            continue

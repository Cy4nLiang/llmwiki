...
compute_conds / stamp_dates / snapshot_manifest,含条件模块块处理);base 侧优先加载实例
tools/init_render.py(旧渲染器语义),新侧加载框架仓 tools/init_render.py。「日期打戳」
「MANIFEST 快照按持有过滤」「条件模块 conds 计算」三处均以 init_render 的同名模块级函数为
单源——框架侧改动只需改 init_render,本文件无同步义务。旧版渲染器(函数化前)缺这些函数时,
hasattr 回退到本文件的 _stamp_dates / _compute_conds / _snapshot_manifest 镜像(仅旧版回退,
口径钉死为函数化前 init_render.main 的内联语义,不随新版演进)。
"""
...
def _compute_conds(cfg: dict) -> dict:
    """【仅旧版回退】条件模块开关镜像:base 侧旧渲染器(函数化前的 init_render)无
    compute_conds 函数时使用;口径钉死为函数化前的三行 config 派生语义,不随新版演进。
    新渲染路径一律走 ir.compute_conds(init_render 单源)。"""
    return {
...
def _stamp_dates(rel: str, text: str, date: str) -> str:
    """【仅旧版回退】日期打戳镜像:base 侧旧渲染器(函数化前的 init_render)无
    stamp_dates 函数时使用;口径钉死为函数化前 init_render.main 的两段内联语义,
    不随新版演进。新渲染路径一律走 ir.stamp_dates(init_render 单源)。
...
def render_tree(ir, tpl_root: Path, cfg: dict, fw_version: str, date: str):
    ...
    渲染函数全部 import 复用:build_plan / compute_slots / render_text / compute_conds /
    stamp_dates(含条件模块块);旧版渲染器缺 compute_conds / stamp_dates 时 hasattr
    回退本文件镜像(仅旧版回退)。"""
    ...
    conds = ir.compute_conds(cfg) if hasattr(ir, "compute_conds") \
        else _compute_conds(cfg)                 # 仅旧版回退
    out, errors, md_items = {}, [], []
    for src, rel in plan:
        if src.suffix == ".md":
            ...
            errors += errs
            md_items.append((rel, text))
        else:
            out[rel] = src.read_bytes()  # skills 附属非 md 文件:字节级比较/拷贝
    if errors:
        raise UpgradeError("渲染失败(%d 处槽位/模块错误):\n%s"
                           % (len(errors), "\n".join("  - %s" % e for e in errors)))
    if hasattr(ir, "stamp_dates"):
        md_items = ir.stamp_dates(md_items, date)
    else:                                        # 仅旧版回退:逐文件镜像
        md_items = [(rel, _stamp_dates(rel, t, date)) for rel, t in md_items]
    out.update(md_items)
    return out, plan
...
def _snapshot_manifest(new_doc, root: Path) -> str:
    """【仅旧版回退/兜底】实例 MANIFEST 快照镜像:新框架渲染器无 snapshot_manifest
    函数(或其返回 None)时使用;口径钉死为函数化前 init_render.main 的快照过滤段
    (保留 dict 外形、indent=1),不随新版演进。正常路径走 ir_new.snapshot_manifest
    (init_render 单源)。"""
...
def main(argv=None) -> int:
    ...
    _snap_fn = getattr(ir_new, "snapshot_manifest", None)   # init_render 单源
    _snap = _snap_fn(new_mf_path, root) if _snap_fn else None
    if _snap is None:                            # 仅旧版回退/兜底:镜像实现
        _snap = _snapshot_manifest(new_doc, root)
    (root / "framework" / "MANIFEST.json").write_text(_snap, encoding="utf-8")

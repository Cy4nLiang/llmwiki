...
<!--BEGIN:multi_facet-->
## 多 facet 约定(config facets 非空时生效)

- facet 字段与取值(config 渲染):<SLOT:facets.fields>;索引徽章:<SLOT:facets.badges>。
- 共享概念跨 facet 累加:同一 concept 页收多个 facet 值的内容,每条事实标 facet 出处;实质覆盖多值的页 tags 加 `cross-<facet key>`,description 明示覆盖面。
- 跨 facet 差异用「对比」关系记录,**不是**「矛盾」(W-ING-3);facet 矩阵/谱系级对照才建 syntheses/ 对照页。
<!--END:multi_facet-->

## 通用要求

- frontmatter 必带 `description:`(分诊触发器:「问 X 读本页」+ 本页独有价值点,≤150 字,W-PAGE-2);`aliases:` 策略随语言约定:<SLOT:lang.clause>
- 页面 token 预算 <SLOT:budgets.page_tokens>(W-PAGE-1):超线拆「精华主页 + `-timeline`/`-appendix` 子页」,主页每个移出节留一行指针。

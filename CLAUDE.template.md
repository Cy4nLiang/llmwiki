<!--
llmwiki 契约模板 · framework <SLOT:framework.version> · 归属:render-once
...
-->
# <SLOT:domain.name> — Wiki Agent 契约
...
**Domain**: <SLOT:domain.description>
...
├── raw/                     ← IMMUTABLE 原始源(分析时只读): <SLOT:pipelines.raw_dirs>
...
- <SLOT:source.naming_rules>
- (W-PAGE-1) 页面预算 ~<SLOT:budgets.page_tokens> tokens:超线拆「精华主页 + 子页」,主页留指针。
...
---
```
(W-PAGE-4) 必填:title / description / type / created / tags / status。源页另带 `source_kind` ∈ <SLOT:source.kind_enum> 及 `raw_file` / `source_url` / `date_published` / `date_ingested` 等采集字段(详见 `.claude/rules/source-page.md`);`updated:` 仅在有意义编辑后 bump(机械回灌不算)。
...
- (W-ING-1) 档位与 touch 下限:<SLOT:ingest.tier_rules>
...
### Rolling 滚动源
<a id="rolling-source"></a>
kind=rolling 的管线:一份源页代表整份滚动源;faithful 快照与 dated 派生分离;版本变化记「演进」,逐版本精选进 canonical 时间线聚合页;查任意版本完整条目直接 grep raw/ 快照。细节见 `.claude/skills/wiki-sync/SKILL.md` 的 rolling 节。
<!--END:rolling_source-->
...
## 会话启动 / Boot-up
<a id="boot"></a>
新会话静默按「稳定→易变」,预算 ~<SLOT:budgets.boot_tokens> tok:读 `wiki/_map.md`(整读;W-LNT-2 硬预算 ≤<SLOT:budgets.map_lines> 行)→ 读 `wiki/overview.md` → `grep '^## \[' wiki/log.md | tail -5`。index / followups 不在启动时整读(见 [硬规则](#rules-hard) grep-only)。不要把这些倒给用户。
...
## 多分面约定 / Multi-facet
<a id="multi-facet"></a>
- 每个源页 frontmatter 必带分面字段:<SLOT:facets.fields>;分面命名前缀见 [命名与体量](#naming);索引徽章:<SLOT:facets.badges>。
- 共享聚合页跨 facet 累加:同一 concept/entity 收多个分面,每条事实标分面出处;实质多分面的页 tags 加 cross 标记,description 明示覆盖面。跨分面 synthesis 最有价值:把多个分面放进同一谱系/矩阵页对比。
- 两级裁决:分面间立场/做法对比先查 concept 页内「跨分面」小节;矩阵/谱系级对照才去 syntheses 对照页。分面差异用「对比」,不是矛盾(见 [交叉引用](#xref))。
<!--END:multi_facet-->

<!--BEGIN:peers-->
## 跨实例引用 / Peers
<a id="peers"></a>
(W-XRF-1) 本实例声明的 peers:<SLOT:peers.list>
- 语法 `[[alias::path/to/slug|显示名]]`,与仓内 wikilink 无歧义共存;**单向**引用,不要求也不制造对方回链(跨仓写入违反 W-ARCH-2)。
- 检索先读 peer 的 `site/agent/pages.jsonl`(含 token 预估)定位,命中后按对方 `_map` 档位读目标页;**跨实例 1 跳封顶,不递归**(对方页内的 peer 链接不再跟进)。
- lint 为 soft:peer 可达则校验目标 slug 存在,断链 warning;不可达 soft warning + 计数,不 fail。peers 是本机路径:不入发布物、不入回流 PR。
<!--END:peers-->
